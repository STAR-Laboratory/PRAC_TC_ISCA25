...
class CommandCounter : public IControllerPlugin, public Implementation {
  ...
  public:
    void init() override { 
      ...
      // Check if the environment variable RAMULATOR_COMMAND_COUNTER_PATH is set
      const char* env_path = std::getenv("RAMULATOR_COMMAND_COUNTER_PATH");

      if (env_path) {
        m_save_path = std::filesystem::path(env_path);  // Use environment variable
        fmt::print("CommandCounter: Using path from environment variable: {}\n", m_save_path.string());        
      } else{
        m_save_path = param<std::string>("path").desc("Path to the trace file").required();  // Use config file
        fmt::print("CommandCounter: Using path from configuration file: {}\n", m_save_path.string());        
      }
