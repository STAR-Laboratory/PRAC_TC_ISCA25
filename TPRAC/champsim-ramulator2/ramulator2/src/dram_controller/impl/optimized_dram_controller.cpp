...
class OPTDRAMController final : public IBHDRAMController, public Implementation {
  ...
  public:
    ...
    void tick() override {
      ...
      // Debug for champsim integration
    //   if (request_found) {
    //     std::cout << "CLK: " << m_clk
    //               << " ADDR: " << req_it->addr
    //               << " Command: " << m_dram->m_commands(req_it->command)
    //               << " Channel: " << req_it->addr_vec[0]
    //               << " Rank: " << req_it->addr_vec[1]
    //               << " BG: " << req_it->addr_vec[2]
    //               << " Bank: " << req_it->addr_vec[3]
    //               << " Row: " << req_it->addr_vec[4]
    //               <<std::endl;
    // }
  ...
  private:
      ...
      if (pending.size()) {
       ...
       if (req.depart <= m_clk) {
          ...
          if (req.depart - req.arrive > 1) {
            ...
            s_read_latency += req.depart - req.arrive; // Measure the latency of being inside the MC
          }
    ...
    bool schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer) {
      ...
      if (!request_found) {
        ...
        // 2.2.2    If no request to be scheduled in the priority buffer, check the read and write buffers.
        if (!request_found) {
